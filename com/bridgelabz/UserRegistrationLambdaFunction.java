...
public class UserRegistrationLambdaFunction {
    ...
    static String phoneNumPattern = "^[0-9]{2} [0-9]{10}$";
    ...
    public void nameIsValid(String name) throws UserRegistrationException{
        try {
            if (checkValid.mathPattern(namePattern, name)){
                ...
                throw new UserRegistrationException("Please Enter a Valid First name with Only One Starting Capital letter");
            }
    ...
    public void eMailIsValid(String email) throws UserRegistrationException{
        try {
            if ( (checkValid.mathPattern(emailPattern1, email)) || (checkValid.mathPattern(emailPattern2, email)) ){
                System.out.println("Entered Correct Email");
            }else {
                throw new UserRegistrationException("Please Enter a Valid Email with Only example.abc@bl.co.in and universal email letter");
            }
        }catch (UserRegistrationException e){
            System.out.println(e.getMessage());
        }
    ...
    public void phoneNumIsValid(String phoneNum) throws UserRegistrationException{
        try {
            if (checkValid.mathPattern(phoneNumPattern, phoneNum)){
                System.out.println("Entered Correct Phone Number");
            }else {
                throw new UserRegistrationException("Please Enter a Valid Mobile Num ex:- 91 9449788942");
            }
        }catch (UserRegistrationException e){
            System.out.println(e.getMessage());
        }
